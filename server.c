...
int main(int argc, char ** argv)
{
    ...
    struct food * first_food = malloc(sizeof(struct food));
    ...
    */
    ...
    wmove(first_win, first_snake->y, first_snake->x);
    waddch(first_win, STAR);
    w_create_food(first_win, first_food, first_cols_x - 1, first_rows_y - 1);    
    wrefresh(first_win);
    wrefresh(second_win);
    sleep(3);
    int counter = 0;
    while(1){
        ...
        switch(key){
            case KEY_UP:
                ...
                w_show_snake(first_win, first_snake, first_food, first_cols_x - 1, first_rows_y - 1, direction->x = 0, direction->y = -1, &points_per_game, &game_state);
                break;
            case KEY_DOWN:
                if(direction->x == 0 && direction->y == -1) break;
                w_show_snake(first_win, first_snake, first_food, first_cols_x - 1, first_rows_y - 1, direction->x = 0, direction->y = 1, &points_per_game, &game_state);
            ...
            case KEY_LEFT:
                ...
                w_show_snake(first_win, first_snake, first_food, first_cols_x - 1, first_rows_y - 1, direction->x = -1, direction->y = 0, &points_per_game, &game_state);
                break;
            case KEY_RIGHT:
                if(direction->x == -1 && direction->y == 0) break;
                w_show_snake(first_win, first_snake, first_food, first_cols_x - 1, first_rows_y - 1, direction->x = 1, direction->y = 0, &points_per_game, &game_state);
                break;
            case ERR:
                w_show_snake(first_win, first_snake, first_food, first_cols_x - 1, first_rows_y - 1, direction->x, direction->y, &points_per_game, &game_state);
                break;
        }
        if(counter++ == 20){
            clean_window(second_win, std_cols_x / 2 - 2, std_rows_y - 1);
            continue;
        }
        fill_send_buffer(send_buffer, first_snake, first_food, points_per_game, &game_state);
        send_all(sock, send_buffer, BUFSIZE, 0);
        recv_all(sock, recv_buffer, BUFSIZE, 0);
        show_another_snake(second_win, recv_buffer, second_snake, &previous_points_per_game, &another_game_state);
    ...
    wmove(first_win, first_rows_y / 2, first_cols_x / 2 - 5);
    waddstr(first_win, "GAME OVER");
    wrefresh(first_win);
    sleep(3);
    wmove(first_win, first_rows_y / 2, first_cols_x / 2 - 5);
    waddstr(first_win, "         ");
    wmove(first_win, first_rows_y / 2, first_cols_x / 2 - 10);
    waddstr(first_win, "Number of points: ");
    waddstr(first_win, buf);
    wmove(second_win, second_rows_y / 2, second_cols_x / 2 - 10);
    waddstr(second_win, "Number of points: ");
    waddstr(second_win, another_buf);
    wrefresh(first_win);
    wrefresh(second_win);
    sleep(2);
    endwin();
